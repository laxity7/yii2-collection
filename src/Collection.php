...
class Collection extends Component implements ArrayAccess, Iterator, Countable
{
     ...
     *                                    This will be passed to [[ArrayHelper::getValue()]].
     *
     * @return int|float the calculated maximum value. 0 if the collection is empty.
     */
     ...
     *                                    This will be passed to [[ArrayHelper::getValue()]].
     *
     * @return int|float the calculated minimum value. 0 if the collection is empty.
     */
     ...
     */
    public function sort(int $direction = SORT_ASC, int $sortFlag = SORT_REGULAR)
    {
        $data = $this->getData();
     ...
     */
    public function sortByKey(int $direction = SORT_ASC, int $sortFlag = SORT_REGULAR)
    {
        $data = $this->getData();
        if ($direction === SORT_ASC) {
            \ksort($data, $sortFlag);
     ...
     */
    public function sortNatural(bool $caseSensitive = false)
    {
        $data = $this->getData();
        if ($caseSensitive) {
            \natsort($data);
     ...
     * @see ArrayHelper::map()
     */
    public function groupBy($groupField, bool $preserveKeys = true)
    {
        $result = [];
        if ($preserveKeys) {
     ...
     */
    public function contains($item, bool $strict = false)
    {
     ...
     * @see Pagination
     */
    public function paginate(Pagination $pagination, bool $preserveKeys = false)
    {
